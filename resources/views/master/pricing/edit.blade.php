        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('pricing.update', $pricing->id) }}" method="POST" id="pricing-form">
                        @csrf
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <select name="ps_unit_id" id="ps_unit_id" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                                    ...
                                    @foreach ($psUnits as $psUnit)
                                        {{-- If the ps_unit_id is selected, mark it as selected --}}
                                        <option value="{{ $psUnit->id }}" {{ old('ps_unit_id', $pricing->ps_unit_id) == $psUnit->id ? 'selected' : '' }}>
                                            {{ $psUnit->type }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="number" name="base_rate" id="base_rate" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value="{{ old('base_rate', number_format($pricing->base_rate, 2, '.', '')) }}" step="any" required>
                                @error('base_rate')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="weekend_surcharge" id="weekend_surcharge" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value="{{ old('base_rate', number_format($pricing->weekend_surcharge, 2, '.', '')) }}" step="any" required>
